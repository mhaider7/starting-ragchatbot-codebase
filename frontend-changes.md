...
### Previously hardcoded values replaced with variables

| Rule | Was | Now |
|---|---|---|
| `.sources-content a` color | `#93c5fd` | `var(--source-link-color)` |
| `.sources-content a:hover` color | `#bfdbfe` | `var(--source-link-hover-color)` |
| `.sources-content a` background | `rgba(37,99,235,0.15)` | `var(--source-link-bg)` |
| `.message-content code/pre` bg | `rgba(0,0,0,0.2)` | `var(--code-bg)` |
| `.welcome-message` box-shadow | `0 4px 16px rgba(0,0,0,0.2)` | `var(--welcome-shadow)` |
| `.error-message` color/bg/border | hardcoded | `var(--error-*)` |
| `.success-message` color/bg/border | hardcoded | `var(--success-*)` |

### Transition coverage expanded

Added `.main-content`, `.error-message`, `.success-message`, `.sources-content a`, `.sources-content span` to the transition selector list so all visible elements animate together during theme switches.

### Removed redundant rules

Removed the duplicate `body.light-mode .sidebar::-webkit-scrollbar-*` block — scrollbar colors now inherit from the shared `--surface` / `--border-color` variables already set in `body.light-mode`.
